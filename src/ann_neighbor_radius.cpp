...
// [[Rcpp::interfaces(r, cpp)]]
// [[Rcpp::export]]
List get_points_indices_in_radius_cpp(const NumericMatrix& data, const NumericMatrix& query_data, int& D, int& ND, int& NQ, int& K, double& EPS,
                                       int& USEBDTREE, const NumericVector& SQRAD) //& const NumericVector& query, 
{
  int query_k; 
  
  IntegerMatrix nn_index(NQ, ND);
  std::fill(nn_index.begin(), nn_index.end(), -1); //NA_REAL 
  ...
  ANNidxArray nn_idx2 = new ANNidx[k];    // Allocate near neigh indices
  ANNdistArray dists2    = new ANNdist[k];   // Allocate near neighbor dists
  int *d_ptr = new int[d]; // pointer for updating ind
  ...
  for(i = 0; i < nq; i++) // Run all query points against tree
  {
    ...
    // first search how many points in the radius (SQRAD[i)
    ANNkdFRPtsInRange[i] =  the_tree->annkFRSearch( pq, SQRAD[i], k, nn_idx, dists,error_bound);

    query_k = ANNkdFRPtsInRange[i];
    nn_idx2 = new ANNidx[query_k];    // Allocate near neigh indices
    dists2 = new ANNdist[query_k];

    // then we search ANNkdFRPtsInRange[i] k nearest points for the current query point pq.  
    the_tree->annkSearch(	// search
        pq,	// query point
        query_k,		// number of near neighbors
        nn_idx2,		// nearest neighbors (returned)
        dists2,		// distance (returned)
        error_bound);	// error bound		

    for(query_k_ind = 0; query_k_ind < query_k; query_k_ind++)
    {
      nn_index(i, query_k_ind) = (int) nn_idx2[query_k_ind]; // put indices in returned array
    }
  }
  ...
  annDeallocPts(data_pts);
  delete [] nn_idx;
  delete [] nn_idx2; // first clean nn_idx2 
  delete [] dists;
  delete [] dists2;
  delete [] d_ptr;
  delete the_tree;
  ...
  return List::create(Rcpp::Named("nn_index") = nn_index, 
                      Rcpp::Named("num_cnts") = ANNkdFRPtsInRange);
}
...
nn2_res <- RANN::nn2(data, k = 5)
dist_k_5 <- nn2_res$nn.dists[, 5]
num_cnts <- get_points_in_radius_cpp(data, data, 185L, 218, 218, 5L, as.double(0.0), 0L, dist_k_5)

test <- get_points_indices_in_radius_cpp(data, data, 185L, 218, 218, 5L, as.double(0.0), 0L, dist_k_5)

num_cnts <- get_points_in_radius_cpp(data, data, 185L, 218, 218, 5L, as.double(0.0), 0L, rep(15, nrow(data)))
test <- get_points_indices_in_radius_cpp(data, data, 185L, 218, 218, 5L, as.double(0.0), 0L, rep(15, nrow(data)))
